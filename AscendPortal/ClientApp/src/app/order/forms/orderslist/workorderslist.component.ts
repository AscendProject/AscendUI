...
@Component({
    ...
    styleUrls: ['./workorderslist.component.scss']
})
export class WorkOrdersListComponent implements OnInit {
    ...
    loadData() {
        ...
        if (this.searchid === '') {
            ...
            this.selectedRow = -1;
            this.orderSandbox.getWorkOrdersList(1, 10);
    ...
    detailsLoad(item) {
        ...
        if (urlParts.length > 3) {
            this.router.navigate(['order', 'work-orders', item.workOrderId, urlParts[urlParts.length - 1]]);
        } else {
            this.router.navigate(['order', 'work-orders', item.workOrderId, 'info']);
        }
    }
    searchData(id) {
