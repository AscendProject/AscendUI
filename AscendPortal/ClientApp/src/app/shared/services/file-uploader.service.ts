...
export class FileUploaderService {
  ...
  private _hfiles: FileQueueObject[] = [];
  private _historyqueue: BehaviorSubject<FileQueueObject[]>;

  constructor(private http: HttpClient,private configService: ConfigService) {
    this._queue = <BehaviorSubject<FileQueueObject[]>>new BehaviorSubject(this._files);
    this._historyqueue = <BehaviorSubject<FileQueueObject[]>>new BehaviorSubject(this._hfiles);

    if (this.configService.isReady) {
          console.log('config service ready');
          this.fileuploadAsPDFUrl = this.configService.serverSettings.vendorDocumentCommandAPIAsPDF;
          this.fileuploadUrl = this.configService.serverSettings.vendorDocumentCommandAPI;
          this.fileDownloadUrl=this.configService.serverSettings.vendorDocumentQueryAPI;
        } else {
  ...
  public get historyQueue() {
    return this._historyqueue.asObservable();
  }
  // public events
  ...
  private _addToQueue(file: any) {
    ...
    const hqueueObj = new FileQueueObject(file);
    ...
    this._hfiles.push(hqueueObj);
    console.log(queueObj.file.name);
    this._historyqueue.next(this._hfiles);
    this._queue.next(this._files);
  }
...
// upload file and report progress
 req = new HttpRequest('POST', this.fileuploadUrl, form, {
  reportProgress: true,
});
    }
    else{
      // upload file and report progress
     req = new HttpRequest('POST', this.fileuploadAsPDFUrl, form, {
      reportProgress: true,
    });
  ...
  private _uploadFailed(queueObj: FileQueueObject, response: HttpErrorResponse) {
    ...
    queueObj.response = response;
    queueObj.uploadFailed = true;
    queueObj.remove();
    this._queue.next(this._files);
    this.onCompleteItem(queueObj, response);
  }

  public getFile(docKey, fileName) {
    return this.http.get(this.fileDownloadUrl  + docKey, { responseType: 'blob' });
